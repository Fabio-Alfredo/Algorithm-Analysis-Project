...
// This struct is used to represent the nodes of the Huffman tree
struct Node
{
    string symbol;
    double probability;
...
// This struct is used to represent the nodes of the minimum priority queue
...
void insert(minPriorityQueue *&root, Node *newNode)
{
    ...
    // Reorder the minimum priority queue
    min_heapify(root);
}
...
void showPreOrder(minPriorityQueue *node)
{
    ...
    if (node)
    {
        cout << "Simbolo: " << node->node->symbol << ", probabilidad : " << node->node->probability << endl;
